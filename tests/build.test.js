...
test("build generates discoverable static pages and indexes", async () => {
  ...
  for(const id of ["json-formatter","json-minifier","json-validator","json-to-csv","csv-to-json","xml-formatter","xml-minifier","html-formatter","html-minifier","css-formatter","css-minifier","sql-formatter","jwt-decoder","uuid-generator","uuid-validator","unix-timestamp-converter","url-parser","query-string-parser","regex-tester","cron-expression-explainer","http-status-code-lookup","mime-type-lookup","color-converter","number-base-converter","json-string-escape"]){
    const item=search.x.find(entry=>entry.i===id);assert.equal(item?.c,"developer",`${id} should be in Developer Tools search`);assert.match(sitemap,new RegExp(`tools/${id}/`));const page=await readFile(path.join(root,"dist","tools",id,"index.html"),"utf8");assert.match(page,/WebApplication/);assert.match(page,/BreadcrumbList/);assert.match(page,/This tool runs in your browser/);
  }
  ...
  for(const id of ["length-converter","weight-converter","temperature-converter","volume-converter","area-converter","speed-converter","time-converter","data-storage-converter","pressure-converter","energy-converter","power-converter","force-converter","torque-converter","angle-converter","frequency-converter","fuel-economy-converter","data-transfer-rate-converter","acceleration-converter","density-converter","cooking-measurement-converter","font-size-converter","flow-rate-converter","voltage-converter","electric-current-converter","illuminance-converter"]){const item=search.x.find(entry=>entry.i===id);assert.equal(item?.c,"unit",`${id} should be in Unit Converter search`);assert.match(sitemap,new RegExp(`tools/${id}/`));const page=await readFile(path.join(root,"dist","tools",id,"index.html"),"utf8");assert.match(page,/WebApplication/);assert.match(page,/BreadcrumbList/);assert.match(page,/This tool runs in your browser/);}
  ...
  for(const id of colorIds){const item=search.x.find(entry=>entry.i===id);assert.equal(item?.c,"color",`${id} should be in Color Tools search`);assert.match(sitemap,new RegExp(`tools/${id}/`));const page=await readFile(path.join(root,"dist","tools",id,"index.html"),"utf8");assert.match(page,/WebApplication/);assert.match(page,/BreadcrumbList/);assert.match(page,/This tool runs in your browser/);assert.equal((page.match(/googletagmanager\.com\/gtag\/js/g)??[]).length,1);assert.equal((page.match(/pagead2\.googlesyndication\.com\/pagead\/js\/adsbygoogle\.js/g)??[]).length,1);}
  ...
  for(const id of securityIds){const item=search.x.find(entry=>entry.i===id);assert.equal(item?.c,"security",`${id} should be in Security search`);assert.match(sitemap,new RegExp(`tools/${id}/`));const page=await readFile(path.join(root,"dist","tools",id,"index.html"),"utf8");assert.match(page,/WebApplication/);assert.match(page,/BreadcrumbList/);assert.match(page,/This tool runs in your browser/);assert.equal((page.match(/googletagmanager\.com\/gtag\/js/g)??[]).length,1);assert.equal((page.match(/pagead2\.googlesyndication\.com\/pagead\/js\/adsbygoogle\.js/g)??[]).length,1);}
