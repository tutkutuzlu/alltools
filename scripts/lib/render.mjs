...
export function renderHome(project) {
  ...
  const body = `<section class="hero hero--home"><div class="container hero__inner"><p class="eyebrow">${tools.length} free browser ${tools.length === 1 ? "tool" : "tools"}</p><h1>The tool you need.<br><span>Right when you need it.</span></h1><p class="lead">Browser utilities for text, developer data, unit conversions, color workflows and security-related generation.</p>${searchForm(categories)}<ul class="trust-list" aria-label="AllTools facts"><li>${iconMarkup("check")}Free to use</li><li>${iconMarkup("check")}No AllTools account</li><li>${iconMarkup("check")}Tool processing stays in your browser</li></ul></div></section>
  ...
  <section class="section section--why"><div class="container"><div class="why-panel"><div class="why-panel__intro"><p class="section-kicker">Choose a family</p><h2>Start with the kind of task.</h2><p>Each category groups tools that share a practical workflow.</p></div><div class="why-grid"><article>${iconMarkup("text")}<h3>Text and developer data</h3><p>Count or clean prose, or format, validate and parse structured values.</p></article><article>${iconMarkup("converter")}<h3>Units and colors</h3><p>Convert measurements and color models, build palettes or check contrast.</p></article><article>${iconMarkup("private")}<h3>Security and generators</h3><p>Create random values or calculate hashes, HMACs and checksums with the appropriate primitive.</p></article></div></div></div></section>`;
...
export function renderCategory(project, category) {
  ...
  <section class="section"><div class="container"><div class="content content--category">${markdownToHtml(category.markdown)}</div><div class="tool-grid">${tools.map((tool) => renderToolCard(site, tool, category)).join("")}</div></div></section>`;
